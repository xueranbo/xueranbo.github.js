...
使用的let的注意事项
  ...
  let不允许在同一个作用域下声明已经存在的变量
	...
	const命令同样有上面let的123条特点，第一所声明的常量只在其所在的代码块内有效，第二声明的常量不会被提升，第三 不能声明已经被声明过的的常量，
	除了这些，在使用const声明的时候需要注意的两点
	  1  声明的时候必须赋值
   ...
   let {a,b,c} = {1,2,3};
		...
		数据结构set
		  ...
		  set类的方法
		   ...
		   数据结构Map
			...
			为了解决这个问题，es6提供了Map数据结构，他类似与对象，也是键值对的集合，但是键的范围不限于字符串，各种类型的值（包裹对象）
			都可以当作键。也就是说，object结构提供了“字符串-值”的对应，Map结构提供了“值-值”的对应，是一种更完善的Hash结构实现，如果
			你需要"键值对"的数据结构，Map比object更适合
		...
		Iterator 和 for 。。。 of 循环
		  ...
		  基本概念
		    在es6中新增了set和map两种数据结构，再加上js之前的原有的数组和对象，这样就有了四种数据集合，平时还可以组合使用他们，定义自己的
		    数据结构，比如数组的成员是Map，Map的成员是对象等，这样就需要一种统一的接口机制，来处理所有不同的数据结构
			
			Iterator就是这样一种机制，它是一种接口，为各种不同的数据结构提供统一的访问机制。任何数据结构只要部署Iterator接口，就可以
			完成遍历操作，而且这种遍历操作是**依次**处理该数据结构的所有成员。
			...
			Iterator的遍历过程
			 ...
			 每一次调用next方法，都会返回数据结构的当前成员的信息。具体老说，就是返回一个value和done两个属相的对象，其中，value
			 属性是当前成员的值，done属性是一个布尔值，表示便利是否结束。
	 ...
	 class 语法
	   ...
	   js语言的传统方法是通过构造函数，定义并生成新对象，是一种基于限制的面向对象系统。这种写法跟传统的面向对象语言，（比如c++和java）差异很大，
	   很容易让新学习这门语言的感到困惑，所以，在es6中新增了类的概念，可以使用class关键字声明一个类，之后以这个类实列化对象
	 ...
	 Miaov这个类除了构造方法，还定义了一个print方法，注意，定义‘类’的方法的时候，前面不需要加上function这个关键字，直接把函数定义放进去就可以
	 了，另外。方法之间不需要逗号隔开，加了会报错
	 ...
	 constructor方法是类的默认方法，通过new命令生成对象，自动调用该方法，一个类必须有constructor方法，如果没有显示定义，一个空的constructor
	...
	函数的扩展
	   ...
	   3 箭头函数
		 ...
		 rest参数形式为（“...变量名”），用于获取函数的多余参数，这样就不需要使用arguments对象了，rest参数搭配的变量是一个数组，该变量将
		 多余的参数放入数组中；
		 ...
		 注意事项
		   箭头函数体内没有自己的this对象，所以在使用的时候，其内部的this就是定义时所在的对象，而不是使用时所在环境的对象,不能给箭头使用
		   call apply bind 去改变内部的this指向
		   function fn(){
		     setTimeout(function(){
			   console.log(this)
			 });
